  ...
  if (active && payload && payload.length) {
    ...
    return (
      ...
      >
        ...
        <p style={{ margin: 0 }}>{`Actual: ${payload[0].value}${
          actualUnit || "%"
        }`}</p>
        <p style={{ margin: 0 }}>{`Forecast: ${payload[1].value}${
          forecastUnit || "%"
        }`}</p>
      </div>
    );
...
const MiniatureChart = ({ data, data1Key, data2Key, increasing }) => {
  return (
    <div style={{ display: "flex", alignItems: "center" }}>
      ...
      <div>
       {increasing ? (<ArrowUpwardIcon sx={{ color: "red" }} />) : (<ArrowDownwardIcon sx={{ color: "green" }}/>)}
      </div>
    </div>
  );
};
